...
func (voteSet *VoteSet) DeepCopy() *VoteSet {
	dc := &VoteSet{
		chainID:       voteSet.chainID,
		height:        voteSet.height,
		round:         voteSet.round,
		signedMsgType: voteSet.signedMsgType,
		valSet:        voteSet.valSet,
		sum:           voteSet.sum,
		maj23:         voteSet.maj23,
		votesByBlock:  voteSet.votesByBlock,
		peerMaj23s:    voteSet.peerMaj23s,
	}
	elems := make([]uint64, len(voteSet.votesBitArray.Elems))
	copy(elems, voteSet.votesBitArray.Elems)
	votesBitArray := &bits.BitArray{
		Bits:  voteSet.votesBitArray.Bits,
		Elems: elems,
	}
	...
	for i := range votes {
		if voteSet.votes[i] == nil {
			...
			vote := *voteSet.votes[i]
			votes[i] = &vote
		}
	}
	dc.votesBitArray = votesBitArray
	dc.votes = votes
	return dc
}
