...
// MakeBlockEx is used in Monaco only.
func MakeBlockEx(height int64, txs [][]byte, hashes [][]byte, lastCommit *Commit, evidence []Evidence) *Block {
	transactions := make(Txs, len(txs))
	for i := 0; i < len(txs); i++ {
