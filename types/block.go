...
func (data *Data) Hash() tmbytes.HexBytes {
	...
	if data.hash == nil {
		txs := make([]Tx, len(data.Txs)+len(data.Hashes))
		for i := range data.Txs {
			txs[i] = data.Txs[i]
		}
		for i := range data.Hashes {
			txs[len(data.Txs)+i] = data.Hashes[i]
		}
		data.hash, _ = QuickHash(txs)
	}
	return data.hash
}
...
func (data *Data) ToProto() tmproto.Data {
	...
	if len(data.Hashes) > 0 {
		hashBzs := make([][]byte, len(data.Hashes))
		for i := range data.Hashes {
			hashBzs[i] = data.Hashes[i]
		}
		tp.Hashes = hashBzs
	}
...
func DataFromProto(dp *tmproto.Data) (Data, error) {
	...
	if len(dp.Hashes) > 0 {
		hashBzs := make([][]byte, len(dp.Hashes))
		for i := range dp.Hashes {
			hashBzs[i] = dp.Hashes[i]
		}
		data.Hashes = hashBzs
	} else {
		data.Hashes = [][]byte{}
