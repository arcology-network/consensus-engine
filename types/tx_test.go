...
import (
	...
	ctest "github.com/arcology-network/consensus-engine/libs/test"
...
func Test_txsHash(t *testing.T) {

	//txs := makeTxs(200000, 370)
	txs := makeTxs(0, 370)
	begintime := time.Now()
	roothash := txs.Hash()

	fmt.Printf("roothash=%x,tim=%v\n", roothash, time.Now().Sub(begintime))

	t.Log("compute success")
}
