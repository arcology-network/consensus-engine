...
func (hvs *HeightVoteSet) DeepCopy() *HeightVoteSet {
	dc := &HeightVoteSet{
		chainID:           hvs.chainID,
		height:            hvs.height,
		valSet:            hvs.valSet,
		...
		peerCatchupRounds: hvs.peerCatchupRounds,
	}
	for k, v := range hvs.roundVoteSets {
		dc.roundVoteSets[k] = RoundVoteSet{
			Prevotes:   v.Prevotes.DeepCopy(),
			Precommits: v.Precommits.DeepCopy(),
		}
	}
	return dc
}
