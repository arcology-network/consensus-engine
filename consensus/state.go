...
type State struct {
	...
	rscMutex tmsync.RWMutex
	rsc      *cstypes.RoundState
...
// GetRoundStateCopy returns a deep copy of the internal consensus state.
func (cs *State) GetRoundStateCopy() *cstypes.RoundState {
	cs.rscMutex.RLock()
	rsc := cs.rsc
	cs.rscMutex.RUnlock()
	return rsc
...
func (cs *State) createProposalBlock() (block *types.Block, blockParts *types.PartSet) {
	...
	return cs.blockExec.CreateProposalBlockEx(cs.Height, cs.state, commit, proposerAddr)
}
...
func (cs *State) addVote(vote *types.Vote, peerID p2p.ID) (added bool, err error) {
	...
	case tmproto.PrecommitType:
		...
		if ok {
			...
			if len(blockID.Hash) != 0 {
				// Before commit, update RoundStateCopy to the latest manually.
				cs.rscMutex.Lock()
				cs.rsc = cs.RoundState.DeepCopy()
				cs.rscMutex.Unlock()
